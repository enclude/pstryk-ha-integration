...
- Confirmed against HA sensor history for 2026-09-10: every `HH:00` run moved `today_net_cost` by exactly `+0.07`, the fixed hourly distribution fee with VAT and nothing else, and the following `HH:05` run then applied the real figure (−1.02 … +1.44). So at `HH:00` the closed hour carries no metered energy at all, not merely incomplete energy.
- **`:05` is sufficient; `:10` is not.** Across all of 2026-09-10 the sensor history contains only `:00` and `:05` entries — HA records state changes only, so the `:10` run never altered a value. Two runs per hour is the right schedule.

**Export data arrives in a bulk evening backfill (source-side, unfixable here)** — separately from the per-hour lag, the day's export readings land in one batch at an irregular evening hour. Measured from `today_energy_export` history: `+2.288 kWh` at 19:00 (2026-09-06), `+4.417` at 22:00 (09-07), `+4.081` at 21:00 (09-08), `+4.488` at 21:00 (09-09), against a baseline of ~0.25 kWh/h in those hours. On 09-09 that step also moved `today_revenue` by `+8.94 PLN` while `today_energy_import` moved `+0.001 kWh`. Consequence: `today_energy_export`, `today_revenue` and `today_net_cost` under-report export all day and jump when the batch lands. No cron schedule can improve this — the data does not exist upstream any earlier. Do not treat the jump as a bug, and do not add runs trying to catch it sooner.
