...
**History DB (SQLite, permanent)** — `/var/tmp/pstryk_history.sqlite` (same `/var/tmp` volume mount as the cache, no extra Docker volume needed). Unlike the cache/`/tmp` logs, this file is never pruned, so it's the durable record for year-plus trend analysis. Requires `sqlite3` on PATH (installed in the Docker image); if absent, the script logs a warning and skips DB writes without failing the run.
...
sqlite3 -header -column /var/tmp/pstryk_history.sqlite \
...
sqlite3 /var/tmp/pstryk_history.sqlite \
